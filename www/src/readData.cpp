...
const char* UUGEAR_ADDR = "UUGear-Arduino-7958-7109";
 ...
 ptr = parseJson();
	...
	if (dev.fd != -1) {
		...
		*/
		...
		char currReadFN[] = "../../../mnt/EXFAT/datalogs/currentReading.txt";
		...
		FILE *datalog_timed; // This should simply create a new file
        	...
        	strftime(datalogTimedFN, sizeof(datalogTimedFN), "../../../mnt/EXFAT/datalogs/datalog_%Y%m%d_%H%M%S.csv", &tmPTime);
        	printf("Creating file \"%s\"\n", datalogTimedFN);
